...
export default async function SearchPage({ searchParams }: PageProps) {
  ...
  const query   = resolvedParams.research?.trim() ?? ''
          ...
          <section style={{ marginTop: 30 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16 }}>
              {userResults.map(user => (
                <a key={user.user_id} href={`/profile/${user.user_id}`} style={{ textDecoration: 'none', textAlign: 'center', width: 120 }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                  ...
                  />
                  <span style={{ color: 'white', fontSize: '0.9em' }}>@{user.user_username}</span>
                </a>
              ))}
            </div>
          </section>
          ...
          <section style={{ marginTop: 30 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16 }}>
              {videoResults.map(video => (
                <a key={video.video_id} href={`/feed?video=${video.video_id}`} style={{ textDecoration: 'none', width: 200 }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                  ...
                  />
                  <p style={{ color: 'white', padding: '6px 0', margin: 0, fontSize: '0.9em' }}>{video.video_title}</p>
                </a>
              ))}
            </div>
          </section>
